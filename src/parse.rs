...
fn extract_declaration(_var: Token, ctx: &mut Context) -> ParseResult<ast::Expr> {
    ...
    let initializer = extract(ctx, parse_expr_id, "an initializer")?;
    Ok(ast::Expr::Declaration { name, typ, initializer })
}

fn extract_while(_while: Token, ctx: &mut Context) -> ParseResult<ast::Expr> {
    let condition = extract(ctx, parse_expr_id, "a condition")?;
    extract_word(ctx, "do")?;
    let body = extract(ctx, parse_expr_id, "a loop body")?;
...
fn extract_conditional(_open: Token, ctx: &mut Context) -> ParseResult<ast::Expr> {
    ...
    let true_branch = extract(ctx, parse_expr_id, "the true branch")?;
    let false_branch = if parse_word(ctx, "else") {
        Some(extract(ctx, parse_expr_id, "the false branch")?)
    }
    else {
        None
    };
...
fn extract_unary(operator: ast::UnaryOp, ctx: &mut Context) -> ParseResult<ast::Expr> {
    extract(ctx, parse_expr_id, "an operand")
        .map(|operand| ast::Expr::UnaryCall { operand, operator })
}
...
fn extract_identifier(token: Token, ctx: &mut Context) -> ParseResult<ast::Expr> {
    match token.view.string(ctx.document) {
        ...
        "return" => Ok(ast::Expr::Return { result: parse_expr_id(ctx)? }),
        "break" => Ok(ast::Expr::Break { result: parse_expr_id(ctx)? }),
...
fn parse_expr(ctx: &mut Context) -> ParseResult<Option<ast::Expr>> {
...
fn parse_expr_id(ctx: &mut Context) -> ParseResult<Option<ast::ExprId>> {
    parse_expr(ctx).map(|expr| expr.map(|expr| ctx.arena.expr.push(expr)))
}

fn extract_function_type(_open: Token, ctx: &mut Context) -> ParseResult<ast::Type> {
    let parameter_types = extract_sequence(ctx, parse_type_id, "a type")?;
    ctx.expect(TokenKind::ParenClose)?;
    ctx.expect(TokenKind::RightArrow)?;
    let return_type = extract(ctx, parse_type_id, "a return type")?;
    Ok(ast::Type::Function { parameter_types, return_type })
}

fn parse_type(ctx: &mut Context) -> ParseResult<Option<ast::Type>> {
    let Some(token) = ctx.lexer.next()
    else {
        return Ok(None);
    };
    match token.kind {
        ...
        TokenKind::Identifier => Ok(Some(ast::Type::Variable { name: ctx.name(token) })),
        _ => {
            ctx.lexer.unlex(token);
            Ok(None)
        }
...
fn parse_type_id(ctx: &mut Context) -> ParseResult<Option<ast::TypeId>> {
    parse_type(ctx).map(|typ| typ.map(|typ| ctx.arena.typ.push(typ)))
}

fn parse_type_annotation(ctx: &mut Context) -> ParseResult<Option<ast::TypeId>> {
    if ctx.consume(TokenKind::Colon) {
        extract(ctx, parse_type_id, "a type").map(Some)
    }
    else {
        Ok(None)
    }
...
fn parse_top_level(ctx: &mut Context) -> ParseResult<Option<ast::TopLevel>> {
    if parse_word(ctx, "fun") {
        extract_function(ctx).map(|fun| Some(ast::TopLevel::Function(fun)))
    }
    else {
...
mod tests {
    ...
    fn conditional() {
        ...
        assert_let!(
            [ast::TopLevel::Expr(ast::Expr::Conditional {
                condition,
                true_branch,
                false_branch
            })] = top_level.as_slice()
    ...
    fn declaration() {
        ...
        assert_let!(
            [ast::TopLevel::Expr(ast::Expr::Declaration {
                name,
                typ,
                initializer
            })] = top_level.as_slice()
    ...
    fn while_loop() {
        ...
        assert_let!(
            [ast::TopLevel::Expr(ast::Expr::WhileLoop {
                condition,
                body
            })] = top_level.as_slice()
        );
    ...
    fn function() {
        ...
        assert_let!(
            [ast::TopLevel::Function(ast::Function {
                name,
                parameters,
                return_type,
                body
        ...
        assert_let!([ast::Expr::Return { result }] = expressions.as_slice());
